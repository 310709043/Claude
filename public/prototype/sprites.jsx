...
(function () {
  const { PixelSprite, AnimatedSprite } = window;

  // ---------- AVATAR HEADS (16x16) ----------
  // Profession-themed pixel mascot heads. 30+ entries.
  // Color code keys vary per sprite for flexibility.
...
`],
  ...
  function drawSkyline(canvas, opts) {
  ...
  function buildingTags(direction = 'neon') {
...
...RRRR....
`;
  const TOMATO_PAL = { R: '#dc2626', G: '#15803d', W: '#fca5a5' };
...
....YYYY....
`;
  const MOON_PAL = { Y: '#fef9c3', B: '#fcd34d' };
...
..WWWWWW..
`;
  const COFFEE_PAL = { W: '#f5f3ff', R: '#7c4a2a', B: '#92400e' };
...
.YYYYYYY.
`;
  const TROPHY_PAL = { Y: '#fcd34d', W: '#fff', B: '#fcd34d' };
...
.NN..
`;
  const NOTE_PAL = { N: '#22d3ee' };

  Object.assign(window, {
    AVATARS, WALKERS, CAT_WALK, TREE, BENCH, LAMP, buildCar, drawSkyline, buildingTags,
    TOMATO_SPRITE, TOMATO_PAL, MOON, MOON_PAL, SUN, SUN_PAL,
    COFFEE, COFFEE_PAL, STAR, STAR_PAL, TROPHY, TROPHY_PAL, NOTE, NOTE_PAL,
  });
})();
